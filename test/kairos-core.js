...
/* ---- mobile console: SAME controls, repositioned by CSS (no duplicate IDs).
   Desktop: .hdr-tools is display:contents, so layout is identical to before.
   Mobile: it becomes a bottom sheet toggled by the ☰ Console button. ---- */
(function(){
  const tb=document.getElementById('btnTools'),tw=document.getElementById('hdrTools');
  if(!tb||!tw)return;
  tb.onclick=function(e){e.stopPropagation();tw.classList.toggle('open');};
  document.addEventListener('click',function(e){
    if(!tw.classList.contains('open'))return;
    if(tw.contains(e.target)||e.target===tb||e.target.closest('#btnTools'))return;
    tw.classList.remove('open');
  });
  ['btnSettings','btnHelp'].forEach(function(id){
    const b=document.getElementById(id);
    if(b)b.addEventListener('click',function(){tw.classList.remove('open');});
  });
})();
