...
/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   The structures above say what each shape IS. This says how you open it, hold
   it and close it: the exact contracts in the order you would enter them, what
   the ticket costs, and the rule that ends the trade.

   Every example is priced on the same imaginary $100 stock so seventeen
   structures can be compared without arithmetic. Real premiums move with
   implied volatility, so read the dollars as proportions rather than quotes.
   ============================================================================ */
window.TDESK.OPT_TIERS = {
  1:['Beginner','One leg, or fully covered. Defined loss, nothing that can surprise you.'],
  2:['Intermediate','Two or more legs. You need to see how they offset before this is safe.'],
  3:['Advanced','Undefined risk or assignment risk. The worst case is not obvious from the payoff diagram.']
};

window.TDESK.OPT_PLAYS = {

'bull-put':{tier:2,
 legs:['SELL 1 put at 0.30 delta · 30 to 45 days out','BUY 1 put $5 lower · same expiry'],
 cost:'Net credit about $1.55 ($155) on a $5 wide spread',
 max:'Gain capped at the $155. Loss is width minus credit, $345.',
 be:'Short strike minus the credit. A 95/90 at $1.55 breaks even at $93.45.',
 enter:'The credit version of a bullish bet, and the one most people should reach for before buying calls. You are paid up front and you win if the stock rises, sits still, or even drifts slightly lower, which is three outcomes out of four. Put the short strike below real support so price has to break something before the trade is in trouble, and sell into elevated IV, because you are a net seller of premium.',
 manage:'Close at 50% of max credit. The last half of the profit takes most of the remaining time and carries all of the remaining risk.',
 exit:'50% of max, or 21 days to expiry, whichever comes first. If the short strike is breached and you still like the thesis, roll down and out for a credit, never for a debit.',
 size:'Risk is $345 per spread, not $155. Size against the width, which is the number that leaves the account if it goes wrong.'},

'bear-call':{tier:2,
 legs:['SELL 1 call at 0.30 delta · 30 to 45 days out','BUY 1 call $5 higher · same expiry'],
 cost:'Net credit about $1.45 ($145) on a $5 wide spread',
 max:'Gain capped at the $145. Loss is width minus credit, $355.',
 be:'Short strike plus the credit. A 105/110 at $1.45 breaks even at $106.45.',
 enter:'The mirror image, and the safer way to express a bearish view than buying puts, because you get paid for time passing rather than paying for it. It wins if price falls, goes nowhere, or rises a little. Put the short strike above a level price has already failed at. Skew works against you here, since calls are relatively cheaper than puts, so the credit is thinner than the bull put for the same delta.',
 manage:'Close at 50% of max credit. Watch for early assignment on the short call around ex-dividend dates, which is the one thing that catches people out on this structure.',
 exit:'50% of max, or 21 days. Roll up and out for a credit if tested and the thesis holds.',
 size:'Risk is $355 per spread. Same rule: size against the width.'},

lc:{tier:1,
 legs:['BUY 1 call · 0.60 to 0.70 delta · 45 to 60 days out'],
 cost:'Debit about $7.00 per contract ($700)',
 max:'Loss capped at the $700. Gain uncapped in theory.',
 be:'Strike plus premium. A $100 strike at $7.00 breaks even at $107.',
 enter:'Buy the deeper delta, not the cheap one. A 0.65 delta call moves about 65 cents per dollar of stock and is mostly intrinsic value, so you are paying for the move rather than for hope. Forty-five days minimum: theta on a 45-day option is roughly a third of a 10-day option, which buys you the right to be early.',
 manage:'At +50% on the premium, sell half. The rest is playing with the house money.',
 exit:'Close by 21 days to expiry regardless. Past that, gamma and theta both accelerate and the trade stops being about your directional read. Hard stop at minus 50%.',
 size:'1 to 2% of the account. The realistic worst case is the entire debit.'},

lp:{tier:1,
 legs:['BUY 1 put · 0.60 to 0.70 delta · 45 to 60 days out'],
 cost:'Debit about $7.00 per contract ($700)',
 max:'Loss capped at the $700. Gain is the strike minus premium if it goes to zero.',
 be:'Strike minus premium. A $100 put at $7.00 breaks even at $93.',
 enter:'Same delta and duration logic as a long call, with one asymmetry: puts cost more than the equivalent call because of skew. The market charges more for downside protection, so your breakeven sits further away than instinct suggests. Buying puts after a selloff has already started is the classic error, since IV has expanded and you are paying peak price for the insurance.',
 manage:'On a fast drop, much of your gain is volatility rather than direction, and that part evaporates on any bounce. Take it while it is there.',
 exit:'Out by 21 days. Take +50% to +100%. Stop at minus 50%.',
 size:'1 to 2%. As portfolio insurance, size against what you are protecting instead.'},

bcs:{tier:2,
 legs:['BUY 1 call at 0.60 delta · 30 to 45 days out','SELL 1 call at 0.30 delta · same expiry'],
 cost:'Net debit about $3.20 ($320)',
 max:'Loss capped at $320. Gain is the width minus the debit, so $680 on a $10 spread.',
 be:'Long strike plus net debit. A 100/110 at $3.20 breaks even at $103.20.',
 enter:'This is the long call with the lottery ticket sold off. The short call cuts your cost by about a third and cuts theta bleed sharply, in exchange for a ceiling. Use it when you have a price target rather than just a direction: put the short strike at the level you actually expect price to reach, because everything above it is money you are handing to somebody else.',
 manage:'The spread does not reach full value until near expiry, so being right early feels underwhelming. That is normal, not a reason to close.',
 exit:'Take 50 to 75% of max gain. Close both legs as one order. Never leg out, because an unhedged short call is a completely different trade.',
 size:'2 to 3%. Defined loss makes this one of the few you can size with confidence.'},

bps:{tier:2,
 legs:['BUY 1 put at 0.60 delta · 30 to 45 days out','SELL 1 put at 0.30 delta · same expiry'],
 cost:'Net debit about $3.50 ($350)',
 max:'Loss capped at $350. Gain is width minus debit, $650 on a $10 spread.',
 be:'Long strike minus debit. A 100/90 at $3.50 breaks even at $96.50.',
 enter:'The bearish mirror of the bull call spread, and skew works in your favour: the put you sell is relatively more expensive than the one you buy, so it costs less than symmetry suggests. Put the short strike at real support, because that is where a decline is most likely to stall.',
 manage:'A gap through both strikes still will not show full value until expiry approaches. Assignment on the short put is possible if it goes deep in the money, especially around a dividend.',
 exit:'50 to 75% of max. Close as one order.',
 size:'2 to 3%.'},
...
csp:{tier:1,
 ...
 max:'Gain capped at the $180. Loss is strike minus premium, so $9,320 if a $95 strike goes to zero.',
 be:'Strike minus premium, $93.20 on a $95 strike.',
 enter:'Only sell puts on something you would be genuinely happy to own at that strike, because that is the actual outcome you are underwriting. The 0.20 to 0.30 delta band is roughly a 70 to 80% chance of expiring worthless, which is the entire business model. Never through earnings unless assignment is the goal: a gap can put you thousands underwater on a $180 credit.',
 manage:'Buy it back at 50% of max profit and redeploy. Squeezing the last half takes most of the remaining time for a fraction of the return.',
 exit:'Close at 50% profit or at 21 days, whichever comes first. If tested and you still want the shares, let it assign and start selling covered calls.',
 size:'Cash-secured means exactly that. If you cannot hold the cash for 100 shares, you are trading a naked put.'},

cc:{tier:1,
 legs:['OWN 100 shares','SELL 1 call at 0.20 to 0.30 delta · 30 to 45 days out'],
 cost:'Credit about $1.60 received ($160)',
 max:'Gain is the premium plus appreciation up to the strike. Loss is the stock going to zero, minus the premium.',
 be:'Your share cost basis minus the premium collected.',
 enter:'The premium is real income, but you are selling your upside for it. Pick a strike above a price you would be content to sell at, because being called away is the deal you signed rather than a failure. Do not sell calls through earnings on a stock you want to keep, and do not sell below your cost basis on a position you are underwater on.',
 manage:'Buy back at 50 to 70% of max profit and resell further out. If it rips through the strike you can roll up and out, but only if the roll genuinely still pays.',
 exit:'Let it expire worthless or get assigned. Both are wins on a position you sized correctly.',
 size:'Per hundred shares owned. The most conservative income structure there is.'},
...
ic:{tier:2,
 ...
 cost:'Credit about $1.60 ($160) on $5 wings',
 max:'Gain capped at $160. Loss is width minus credit, $340.',
 be:'About $93.40 and $106.60 on the example.',
 enter:'You are betting on a range, so it needs high implied volatility to be worth doing: IV rank above 50 means the market is paying you a wider range for the same risk. The 0.16 delta shorts sit near a one standard deviation move, roughly a 68% chance of finishing between them. Liquid underlyings only, because four legs means four bid-ask spreads and an illiquid chain eats the whole credit on entry and exit.',
 manage:'Close at 50% of max profit. Risk-reward gets worse the longer you hold. Rolling the untested side closer collects more credit but narrows the profit zone on a trade already going against you.',
 exit:'50% of max, or 21 days. Do not hold four legs into expiration week hoping.',
 size:'Risk is $340 per condor. Size by that, never by the credit.'},

ib:{tier:2,
 legs:['SELL 1 call at the money','SELL 1 put at the same strike','BUY 1 call $10 higher','BUY 1 put $10 lower','same expiry · 30 to 45 days out'],
 cost:'Credit about $4.00 ($400) on $10 wings',
 max:'Gain $400 if it pins the short strike exactly. Loss is width minus credit, $600.',
 be:'$96 and $104 on the example.',
 enter:'An iron condor with the shorts collapsed onto one strike, which is why it pays far more and wins far less often. You need a genuine pin thesis: a stock parked at a big open-interest strike, a post-earnings drift into a known level, or a low realised-volatility regime. Sell into high IV and take it off before the move you bet against has time to happen.',
 manage:'Profit comes fast if price sits still, and it is at max risk the moment it does not. There is no drifting back.',
 exit:'25 to 35% of max profit is the standard target. Close on any decisive break of a breakeven rather than hoping.',
 size:'Half what you would size an iron condor. Touching a short strike is close to certain.'},

ss:{tier:3,
 legs:['SELL 1 call at 0.16 delta','SELL 1 put at 0.16 delta','same expiry · 30 to 45 days out','NO protective wings'],
 cost:'Credit about $3.20 ($320)',
 max:'Gain capped at $320. Loss theoretically unlimited on the call side.',
 be:'About $91.80 and $108.20.',
 enter:'The iron condor with the insurance removed, and the insurance is what turns a bad week into a survivable one. Requires margin approval, a large account, and a written plan for a 2x credit loss before you enter. Never on a single stock through earnings, never on anything that can be acquired or squeezed. If you cannot state your maximum loss as a number, trade the iron condor instead.',
 manage:'Defend at 2x the credit received. Roll the tested side out in time for a credit, or close. Rolling forever is deferral, not management, and it is how a $320 credit becomes a five-figure loss.',
 exit:'50% of max, or 21 days. Close at 2x credit loss without negotiating.',
 size:'If one position can meaningfully damage the account it is too big. Most retail accounts should not be here.'},

straddle:{tier:2,
 legs:['BUY 1 call at the money','BUY 1 put at the same strike','same expiry · 30 to 45 days out'],
 cost:'Debit about $8.50 total ($850)',
 max:'Loss $850 if it finishes exactly at the strike. Gain uncapped on the upside.',
 be:'$91.50 and $108.50.',
 enter:'You are buying a move without picking a side, and the price of that is the stock must travel more than 8% just to get you flat. Only worth it when the expected move priced into the options is smaller than the move you actually expect, which usually means low IV rank ahead of a catalyst the market has not woken up to. Buying a straddle the day before earnings is the classic trap: IV is at its annual peak and the crush the next morning routinely wipes out more than the gap creates.',
 manage:'When one leg doubles, consider selling it and letting the other ride at zero further cost.',
 exit:'Out well before 21 days. Two long at-the-money options is the fastest theta bleed in this list.',
 size:'1 to 2%. The most likely single outcome is a partial loss.'},

strangle:{tier:2,
 legs:['BUY 1 call at 0.30 delta','BUY 1 put at 0.30 delta','same expiry · 45 to 60 days out'],
 cost:'Debit about $4.20 total ($420)',
 max:'Loss $420 if it finishes between the strikes. Gain uncapped above.',
 be:'About $89.80 and $110.20.',
 enter:'Half the cost of a straddle and it needs a bigger move to pay. Use it when you expect something violent rather than merely significant, and give it more time, because the strikes are further away and need room to be reached.',
 manage:'Same as the straddle: sell the winning leg into a sharp move rather than waiting for perfection.',
 exit:'Out before 21 days.',
 size:'1 to 2%.'},

cal:{tier:2,
 legs:['SELL 1 call at the money · 30 days out','BUY 1 call at the same strike · 60 days out'],
 cost:'Net debit about $1.90 ($190)',
 max:'Loss capped near $190. Gain depends on IV when the front leg expires, typically two to three times the debit at best.',
 be:'A range around the strike that widens with back-month volatility, not a fixed number.',
 enter:'You are selling fast time and buying slow time, so it profits when the stock sits still and when back-month IV rises. Enter when the front month is expensive relative to the back, which happens ahead of a known event inside the near expiry. Put the strike where you expect the stock to be, because a calendar is most valuable when price pins it.',
 manage:'Roll the short leg forward after it decays out, converting into a new calendar at a lower cost basis. If the stock runs away in either direction, it loses regardless of which way.',
 exit:'Close when the front leg has given up most of its value, typically 25 to 50% of the debit as profit.',
 size:'1 to 2%. Vega exposure means an IV collapse hurts even when price behaves.'},

diag:{tier:3,
 legs:['SELL 1 call at 0.30 delta · 30 days out','BUY 1 call at a lower strike · 60 to 90 days out'],
 cost:'Net debit about $4.50 ($450)',
 ...
 enter:'A calendar with a directional tilt, and the base of the poor mans covered call: the long back-month call stands in for 100 shares at a fraction of the capital. The long leg should be deep enough in the money, around 0.75 to 0.80 delta, that it behaves like stock. Critical rule: the strike gap between your legs must exceed the debit paid, or an adverse assignment locks in a loss you cannot escape.',
 manage:'Sell a new short call each cycle against the long. Roll the long out well before it enters its own steep decay.',
 exit:'Close the whole structure if the long leg falls below 0.60 delta, because it has stopped standing in for stock.',
 size:'2 to 3%, and treat the long leg as a stock position for exposure purposes.'},

fly:{tier:3,
 legs:['BUY 1 call $5 below the target','SELL 2 calls at the target','BUY 1 call $5 above the target','same expiry · 20 to 30 days out'],
 cost:'Net debit about $1.10 ($110)',
 max:'Loss capped at $110. Gain is wing width minus debit, $390.',
 be:'About $96.10 and $103.90.',
 ...
 manage:'It is worth very little until the final week, so early paper losses mean little. That patience is also the trap, since there is no time left to recover if you are wrong.',
 exit:'Take anything above 50% of max. Waiting for the perfect pin usually converts a good win into a full loss.',
 size:'Under 1%. The low win rate is priced into the payoff, so many small tickets rather than one large one.'},

bwb:{tier:3,
 legs:['BUY 1 put $10 below the target','SELL 2 puts at the target','BUY 1 put $25 below the target','same expiry · 30 to 45 days out'],
 cost:'Often a small credit or near zero debit',
 max:'Gain is roughly the narrow wing plus any credit. Loss is the wide wing minus the narrow wing minus the credit, a substantially larger number.',
 be:'One breakeven on the wide side. The narrow side often has none, which is the point.',
 enter:'A butterfly with one wing pushed out, financing the structure so it costs nothing or pays you to enter. The result has no risk at all in one direction and a real, larger risk in the other. Put the wide wing where you least expect price to go. It is a genuinely good shape, and it sits at advanced because the payoff diagram makes the risk look smaller than it is.',
 manage:'Watch the wide side. That is where the loss lives, and it can exceed the credit many times over.',
 exit:'Take 25 to 50% of max. Close on any decisive move toward the wide wing rather than negotiating with it.',
 size:'Size against the wide-wing loss, never against the credit received.'},

jade:{tier:3,
 legs:['SELL 1 put at 0.25 delta','SELL 1 call at 0.25 delta','BUY 1 call $5 above the short call','same expiry · 30 to 45 days out'],
 cost:'Credit about $2.90 ($290)',
 max:'Gain capped at the credit. Loss undefined below the short put, defined above.',
 be:'Short put strike minus the credit, about $92.10.',
 enter:'The one rule that makes this a jade lizard rather than a random three-legged position: the total credit must exceed the width of the call spread. Collect $2.90 on a $5 wide spread and you still have upside risk. Collect $5.10 on a $5 wide spread and there is literally no way to lose if the stock goes up, at any price, forever. That is the entire reason the structure exists. Sell into elevated IV on something you would not mind owning.',
 manage:'All the risk is below. Manage the short put exactly as a cash-secured put: roll down and out for a credit, or take assignment on a name you wanted anyway.',
...
ratio:{tier:3,
 ...
 enter:'You want a measured move to a level and nothing beyond it. Because you sell two and buy one it often costs nothing, and that free entry is exactly what makes it dangerous: the second short call is naked. A gap through the short strike produces losses with no ceiling. Never ahead of earnings, a takeover rumour, or anything with squeeze potential.',
 manage:'Decide a hard price above the short strike at which you buy back a short call, before entry rather than during the move.',
 ...
 size:'The smallest size in this list. If a gap up would meaningfully damage the account, it is too large.'}
};
