...
function renderTrinity(){
  ...
  list.forEach((sym,slotIdx)=>{
    ...
    /* The chart module owns this node and hands back the SAME element every
       time, so the LightweightCharts instance inside it survives the pillar
       rebuild. Creating it here instead would destroy the chart on every
       refresh, which is what caused the black flash. */
    if(window.KairosCosmosChart)p.appendChild(window.KairosCosmosChart.stageFor(sym));
    el.appendChild(p);
    p.querySelector('.ticker-sel').onchange=e=>{
      const old=e.target.dataset.old,neu=cleanSym(e.target.value);
      if(!neu){e.target.value=old;return;}
